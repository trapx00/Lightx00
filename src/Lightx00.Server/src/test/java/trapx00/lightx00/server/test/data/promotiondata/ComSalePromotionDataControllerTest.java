...
public class ComSalePromotionDataControllerTest {
    ...
    public void query() throws Exception {
        try {
            dao.create(promotion);
            assertEquals(1, service.queryPromotion(new ComSalePromotionQueryVo().eq("id", promotion.getId()).castBack()).length);
            assertEquals(0, service.queryPromotion(new ComSalePromotionQueryVo().eq("id", "12").castBack()).length);
        } finally {
            dao.deleteById(promotion.getId());
        }
    }
