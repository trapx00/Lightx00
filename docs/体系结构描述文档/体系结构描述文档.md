    ...
    - [5. 接口视角](#5-接口视角)
        ...
        - [5.3 业务逻辑层的分解](#53-业务逻辑层的分解)
            ...
            - [5.3.2 业务逻辑模块的接口规范](#532-业务逻辑模块的接口规范)
                ...
                - [userblservice的接口定义](#userblservice的接口定义)
        ...
        - [5.4 数据层的分解](#54-数据层的分解)
            ...
            - [5.4.2 数据层模块的接口规范](#542-数据层模块的接口规范)
                ...
                - [userdataservice的接口定义](#userdataservice的接口定义)
...
| 接口名称                                     | 语法                                       | 前置条件                     | 后置条件                    |
| ---------------------------------------- | ---------------------------------------- | ------------------------ | ----------------------- |
| promotiondataserve.PromotionManagement.submit | `public ResultMessage submit(PromotionPO promotion);` | 促销策略所有属性有效，与已有促销策略不产生冲突。 | 保存组合商品降价促销策略持久化对象数据。    |
| promotiondataservice.PromotionManagement.queryPromotion; | `promotiondataservice.PromotionManagement.queryPromotion(Date[] timeRange,PromotionType promotionType,Client client,double totalPrice);` | 输入有效，其中值为null的字段表示无限制。   | 返回符合筛选条件的Promotion对象数组。 |
...
| 接口名称                                     | 服务名              |
| ---------------------------------------- | ---------------- |
| `promotiondataservice.PromotionManagement.submit(PromotionPO promotion);` | 提交新促销策略。         |
...
| logservice.Logger.log     | `public ResultMessage log(LogSeverity severity, String content)` | content非空。 | 日志保存，持久化信息已经保存。 |
