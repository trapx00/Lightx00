...
| 张凌哲  | 2017/12/20 | 增加用例备份为云。   | V1.3 |
| 张凌哲  | 2017/12/30 | 修改接口。       | V1.4 |
| 梁楠   | 2017/1/1   | 修改接口。       | v1.5 |
...
| `commoditydataservice.CommodityDataService.getAllCommodity()` | 得到单一化持久对象。          |
| `logbl.LogBl.log(LogSeverity severity, String content)` | 记录日志。               |
| `commoditydataservice.CommodityDataService.getId()` | 得到新商品id。            |
| `commoditydataservice.CommoditySortDataService.query(CommoditySortQueryVo commoditySortQueryVo)` | 根据ID和/或名称查找单一持久化对象。 |
| `commoditydataservice.CommoditySortDataService.add(CommoditySortPo commoditySort)` | 添加单一化持久对象。          |
| `commoditydataservice.CommoditySortDataService.modify(CommodityPo updateCommoditySort)` | 修改添加单一化持久对象。        |
| `commoditydataservice.CommoditySortDataService.delete(CommoditySortPo commoditySort)` | 删除单一化持久对象。          |
| `commoditydataservice.CommoditySortDataService.display()` | 查找所有单一持久化对象。        |
| `commoditydataservice.CommoditySortDataService.display(CommoditySortPo commoditySort)` | 查找单一持久化对象。          |
| `commoditydataservice.CommoditySortDataService.getId()` | 得到新商品分类id。          |
...
| InventoryCheckBlService.export           | `public ResultMessage export(String path，String name);` | 已经完成库存盘点。         | 导出Excel到指定位置。                            |
...
| InventoryGiftBlService.saveAsDraft       | `public ResultMessage saveAsDraft(InventoryGiftVo bill);` | 单据信息非空。           | 保存草稿，持久化信息已经保存。                          |
...
| InventoryWarningBlService.saveAsDraft    | `public ResultMessage saveAsDraft(InventoryDetailBillVo bill);` | 单据信息非空。           | 保存草稿，持久化信息已经保存。                          |
...
| 接口名称                  | 语法                                       | 前置条件          | 后置条件                  |
| --------------------- | ---------------------------------------- | ------------- | --------------------- |
| AuditBlService.query  | `public ResultMessage  show (AuditIdQueryVo query);` | 有处于提交状态的单据单据。 | 返回符合条件的单据。            |
| AuditBlService.reject | `public ResultMessage reject(AuditIdVo billInfo);` | 用户已经查看了单据内容。  | 拒绝通过审批，持久化更新涉及的对象的数据。 |
| AuditBlService.pass   | `public ResultMessage pass(AuditIdVo billInfo);` | 用户已经查看了单据内容。  | 通过审批，持久化更新涉及的对象的数据。   |
...
| `approvaldataservice.AuditDataService.reject(AuditIdPo billInfo)` | 拒绝通过审批。      |
| `approvaldataservice.AuditDataService.pass(AuditIdPo billInfo)` | 通过审批。        |
...
| ClientManagementBlService.queryById   | `public ClientVo queryById(String id);`  | 查询id合法。     | 返回符合id的客户信息。 |
...
| `clientdataservice.ClientManagementDataService.queryById(String id)` | 根据id查询客户信息。   |
...
| SaleBillBlService.queryPromotion         | `public PromotionVoBase[] queryPromotion(SaleBillVo vo); ` | 无。          | 获得促销。       |
| SaleBillBlService.querySaleBill          | `public SaleBillVo[] querySaleBill(SaleBillQueryVo query);` | 查询条件合法。     | 查询销售单据。     |
...
| SaleBillRefundBlService.querySaleRefundBill | `public SaleRefundBillVo[] querySaleRefundBill(SaleRefundBillQueryVo query); ` | 查询条件合法。     | 获得销售退货单。    |
...
| InitialEstablishmentDataService.activate | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。    | 系统修改单据状态为已入账，持久化信息已经保存。               |
| InitialEstablishmentDataService.query    | `public SystemSnapshotVo[] query(SystemSnapshotQueryVo query);` | 输入查询条件有效。        | 返回符合筛选条件的付款单。                         |
...
| 接口名称                                     | 语法                                       | 前置条件            | 后置条件                                  |
| ---------------------------------------- | ---------------------------------------- | --------------- | ------------------------------------- |
| InventoryGiftDataService.submit          | `public ResultMessage submit(InventoryGiftPo bill);` | 输入有效            | 提交成功则返回ResultMessage.Success          |
| InventoryGiftDataService.activate        | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。   | 系统修改单据状态为已入账，持久化信息已经保存                |
| InventoryGiftDataService.abandon         | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。   | 系统修改单据状态为已经废弃，持久化信息已经保存。              |
| InventoryGiftDataService.approvalComplete | `public ResultMessage approvalComplete(String billId, BillState billState);` | bill id有效，状态有效。 | 改变单据状态，持久化信息已经保存。                     |
| InventoryGiftDataService.getId           | `public String getId();`                 | 无。              | 返回下一张单据的ID。                           |
| InventoryGiftDataService.query           | `public InventoryGiftPo[] query(InventoryGiftQueryVo query)` | 输入有效。           | 返回符合条件的Po。                            |
| InventoryWarningDataService.submit       | `public ResultMessage submit(InventoryDetailBillPo bill);` | 单据属性有效。         | 提交新单据，返回提交是否成功。                       |
| InventoryWarningDataService.modify       | `public ResultMessage modify(String id, double warningValue);` | 同样的po在数据中存在     | 更新po,修改警戒值。                           |
| InventoryWarningDataService.activate     | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。   | 系统修改对应库存信息，修改单据状态为已入账，持久化信息已经保存。      |
...
| InventoryWarningDataService.query        | `public InventoryDetailBillPo[] query(InventoryBillQueryVo query)` | 输入有效。           | 返回符合条件的Po。                            |
| InventoryWarningDataService.approvalComplete | `public ResultMessage approvalComplete(String billId, BillState billState);` | bill id有效，状态有效。 | 改变单据状态，持久化信息已经保存。                     |
...
| 接口名称                    | 语法                                       | 前置条件              | 后置条件            |
| ----------------------- | ---------------------------------------- | ----------------- | --------------- |
| AuditDataService.query  | `public AuditIdVo[] query(AuditIdQueryVo query);` | po中存在处于提交状态的单据数据。 | 返回待审批单据。        |
| AuditDataService.reject | `public ResultMessage reject(AuditIdPo bill);` | 无。                | 修改并保存单据持久化对象数据。 |
| AuditDataService.pass   | `public ResultMessage pass(AuditIdPo bill);` | 无。                | 修改并保存单据持久化对象数据。 |
...
| ClientManagementDataService.queryById | `public ClientPO query(String id);`      | 查询id合法。     | 返回符合id的用户信息。 |
...
| logindataservice.FaceIdAuthenticationDataService.authenticate | `public String authenticate(byte[] faceImage)` | 参数为有效图片信息。 | 系统已经登录或者登录失败。 |
...
```java
class ClientPromotion extends PromotionPoBase{
    int clientLevel;
    double couponPrice;
    double onSalePrice;
}
...
```java
class ComSalePromotion extends PromotionPoBase{
    double salePrice;
}
```

```java
class TotalPricePromotion extends PromotionPoBase{
    double couponPrice;
    double totalPrice;
}
```
...
```java
class AdminStaffPo extends EmpolyeePo{
}
```
