...
| InventoryGiftDataService.abandon         | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。 | 若存在则返回符合条件的InventoryGiftPo，否则返回NULL。     |
| InventoryGiftDataService.approvalComplete | `public ResultMessage approvalComplete(String billId, BillState billState);` | 输入有效          | 若存在则返回符合条件的InventoryGiftPo，否则返回NULL。     |
| InventoryGiftDataService.getId           | `public String getId();`                 | 输入有效          | 若存在则返回符合条件的InventoryGiftPo，否则返回NULL。     |
