...
| AuditBlService.pass   | `public ResultMessage pass(BAuditIdVo billInfo);` | 用户已经查看了单据内容。  | 通过审批，持久化更新涉及的对象的数据。   |
...
| CommodityDataService.query       | `public CommodityPo[] query(CommodityQueryVo commodityQueryVo);` | 查询输入格式合法。     | 返回符合查询条件的数据。 |
...
| CommoditySortDataService.modify  | `public ResultMessage modify(CommoditySortPo  CSort);` | 同样的po在数据中存在。  | 更新po。        |
| CommoditySortDataService.query   | `public CommoditySortPo[] query(CommoditySortQueryVo commoditySortQueryVo);` | 无 。           | 返回符合查询条件的数据。 |
...
| 接口名称                                     | 语法                                       | 前置条件            | 后置条件                                  |
| ---------------------------------------- | ---------------------------------------- | --------------- | ------------------------------------- |
| InventoryCheckDataService.check          | `public InventoryViewPo[] submit(InventoryViewQueryVo inventoryViewQueryVo);` | 当天有库存进出且输入有效。   | 返回对应的InventoryViewPo。                 |
| InventoryCheckDataService.picture        | `public InventoryPicturePo[] submit(InventoryPictureQueryVo inventoryPictureQueryVo);` | 当天有库存快照。        | 返回对应的库存快照InventoryPicturePo。          |
...
| 接口名称                    | 语法                                       | 前置条件              | 后置条件            |
| ----------------------- | ---------------------------------------- | ----------------- | --------------- |
| AuditDataService.query  | `public AuditIdVo[] query(AuditQueryVo query);` | po中存在处于提交状态的单据数据。 | 返回待审批单据。        |
| AuditDataService.reject | `public ResultMessage reject(AuditIdPo billInfo);` | 无。                | 修改并保存单据持久化对象数据。 |
| AuditDataService.pass   | `public ResultMessage pass(AuditIdPo billInfo);` | 无。                | 修改并保存单据持久化对象数据。 |
