...
| `financedataservice.SaleDetailDataService.query(SaleDetailQueryVo query)` | 根据条件查找单据。    |
| `financedataservice.TradeHistoryDataService.query(TradeHistoryQueryVo query)` | 根据条件查找单据。    |
...
| SaleDetailDataService.query      | `public SaleDetailVo query(SaleDetailQueryVo query);` | 输入有效。            | 返回符合条件的商品销售记录，若某一属性为null则为无限制。        |
| TradeHistoryDataService.query    | `public BillPo[] query(TradeHistoryQueryVo query);` | 输入有效。            | 返回符合条件的商品销售记录，若某一属性为null则为无限制。        |
