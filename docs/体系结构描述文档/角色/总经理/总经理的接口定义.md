...
| 接口名称                                     | 服务名          |
| ---------------------------------------- | ------------ |
| `approvaldataservice.Audit.query(BillQueryVo query)` | 得到待审批的持久化对象。 |
| `approvaldataservice.Audit.reject(BillPO bill)` | 拒绝通过审批。      |
| `approvaldataservice.Audit.pass(BillPO bill)` | 通过审批。        |
...
| 接口名称                             | 语法                                       | 前置条件              | 后置条件            |
| -------------------------------- | ---------------------------------------- | ----------------- | --------------- |
| approvaldataservice.Audit.query  | `public BillVO[] query(BillQueryVo query)` | po中存在处于提交状态的单据数据。 | 返回待审批单据。        |
...
| promotionblservice.PromotionManagement.getId | `public String getId();`                 | 新建促销策略。                             | 返回当前促销策略的ID。            |
...
| `promotiondataservice.PromotionManagement.queryPromotion(PromotionQueryVO query)` | 根据不同的筛选条件查找促销策略。 |
| `promotiondataservice.Promotion.delete(PromotionPOBase promotion)` | 删除单一持久化对象。       |
| `promotiondataservice.PromotionManagement.queryCommodity(CommodityQueryVO query)` | 根据商品标识查找商品。      |
| `promotiondataservice.PromotionManagement.getId()` | 返回当前促销策略ID。      |
...
| promotiondataservice.PromotionManagement.queryPromotion; | `public PromotionPOBase[] queryPromotion(PromotionQueryVO query);` | 输入有效，其中值为null的字段表示无限制。   | 返回符合筛选条件的Promotion对象数组。 |
...
| promtiondataservice.PromotionManagement.queryCommodity; | `public CommodityPO[] queryCommodity(CommodityQueryVO query);` | 输入有效，其中值为null的字段表示无限制。   | 返回符合筛选条件的Commoidty对象数组。 |
| promotiondataservice.PromotionManagement.getId | `public String getId();`                 | 无。                       | 返回当前促销策略对象的ID。          |
...
| logblservice.Log.getId | `public String getId();`                 | 记录新的操作日志。      | 返回当前日志记录对象的ID。 |

需要的接口

| 接口名称                                     | 服务名            |
| ---------------------------------------- | -------------- |
| `logdataservice.Log.query(LogQueryVO query)` | 得到持久化的操作日志对象。  |
| `logdataservice.Log.getId()`             | 得到当前日志记录对象的ID。 |
