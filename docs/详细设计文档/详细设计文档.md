...
- [2.结构视角](#2结构视角)
    ...
    - [2.3 数据层的分解](#23-数据层的分解)
        ...
        - [2.3.15 faceid包](#2315-faceid包)
            - [2.3.15.1 概述](#23151-概述)
            - [2.3.15.2 整体架构](#23152-整体架构)
            - [2.3.15.3 设计](#23153-设计)
                - [2.3.15.3.1 设计图](#231531-设计图)
                - [2.3.15.3.2 各个类的职责](#231532-各个类的职责)
                - [2.3.15.3.3 内部类的接口规范](#231533-内部类的接口规范)
- [3. 依赖视角](#3-依赖视角)
...
| 张凌哲  | 2017/12/20 | 增加用例备份为云，修改接口及VOPO。 | V2.1 |
...
| FaceIdLoginUiController | 负责人脸识别功能的展示和功能。 |
...
**FaceIdLoginUiController**

需要的接口

| 接口名称                                     | 服务名       |
| ---------------------------------------- | --------- |
| `loginblservice.FaceIdAuthenticationBlService.authenticate(byte[] faceImage`) | 通过人脸照片登录。 |
...
| InventoryPictureUiController   | 负责库存快照界面的展示和功能。    |
...
| 接口名称                                     | 服务名         |
| ---------------------------------------- | ----------- |
| `inventoryblservice.InventoryCheckBlService.getInventoryView(Date beginTime, Date endTime);` | 提供时间段内库存变化。 |

**InventoryPictureUiController**

需要的接口

| 接口名称                                     | 服务名       |
| ---------------------------------------- | --------- |
| `inventoryblservice.InventoryCheckBlService.getInventoryPicture();` | 提供当日库存快照。 |
| `inventoryblservice.InventoryCheckBlService.export(String path,String name);` | 导出Excel。  |
...
| 接口名称                             | 语法                                       | 前置条件            | 后置条件        |
| -------------------------------- | ---------------------------------------- | --------------- | ----------- |
| ContinueWritable.continueWriting | `public ExternalLoadableUiPackage continueWriting(InventoryBillVo bill);` | 输入有效的用于继续填写的单据。 | 初始化传入单据的内容。 |
...
| 接口名称                                     | 服务名      |
| ---------------------------------------- | -------- |
| `commodityui.CommoditySelection.showCommoditySelectDialog();` | 选择商品。    |
| `inventoryblservice.InventoryGiftBlService.getId();` | 得到ID。    |
| `inventoryblservice.InventoryGiftBlService.saveAsDraft(InventoryGiftVo inventoryGiftVo);` | 保存草稿。    |
...
| `commodityblservice.CommodityInfo.modify(CommodityVo commodity);` | 修改警戒值。 |
| `inventoryblservice.InventoryWarningBlService.getId();` | 提供ID。  |
| `commodityblservice.CommodityInfo.queryCommodity();` | 查询商品。  |
| `commodityui.CommoditySelection.showCommoditySelectDialog();` | 选择商品。  |
...
| 接口名称                             | 语法                                       | 前置条件        | 后置条件        |
| -------------------------------- | ---------------------------------------- | ----------- | ----------- |
| ContinueWritable.continueWriting | `public ExternalLoadableUiPackage continueWriting(PurchaseBillVo bill);` | 需要继续填写单据信息。 | 初始化传入单据的内容。 |
...
| 接口名称                             | 语法                                       | 前置条件        | 后置条件        |
| -------------------------------- | ---------------------------------------- | ----------- | ----------- |
| ContinueWritable.continueWriting | `public ExternalLoadableUiPackage continueWriting(PurchaseRefundBillUiController bill);` | 需要继续填写单据信息。 | 初始化传入单据的内容。 |
...
| 接口名称                             | 语法                                       | 前置条件        | 后置条件        |
| -------------------------------- | ---------------------------------------- | ----------- | ----------- |
| ContinueWritable.continueWriting | `public ExternalLoadableUiPackage continueWriting(SaleBillVo bill);` | 需要继续填写单据信息。 | 初始化传入单据的内容。 |
...
| `salebl.SaleBillBlService.queryPromotion(SaleBillVo); ` | 获得可用的销售策略。 |
...
| 接口名称                             | 语法                                       | 前置条件        | 后置条件        |
| -------------------------------- | ---------------------------------------- | ----------- | ----------- |
| ContinueWritable.continueWriting | `public ExternalLoadableUiPackage continueWriting(SaleRefundBillVo bill);` | 需要继续填写单据信息。 | 初始化传入单据的内容。 |
...
| 类名                    | 职责                   |
| --------------------- | -------------------- |
| CommodityUiController | 负责商品分类和商品管理界面的展示和功能。 |
| CommoditySelection    | 提供给其他人员商品的展示和功能。     |
...
| 接口名称                                     | 服务名   |
| ---------------------------------------- | ----- |
| `commodityblservice.CommodityInfo.queryCommodity(CommdoityQueryVo commodityQueryVo);` | 查询商品。 |
...
| ---------------------------------------- | ------- |
| `clientui.ClientInfoUi.showClientSelectDialog() ` | 选择客户。   |
| `adminui.EmployeeSelection.showEmployeeSelectDialog()` | 选择操作员。  |
...
| 接口名称                                     | 语法                                       | 前置条件 | 后置条件          |
| ---------------------------------------- | ---------------------------------------- | ---- | ------------- |
| BankAccountSelection.showBankAccountSelectDialog | `public void showBankAccountSelectDialog(Consumer<BankAccountVo> callback);;` | 无。   | 弹出选择银行账户的窗口。  |
| BankAccountSelection.queryId             | `public BankAccountVo queryId(int id);`  | 无。   | 返回ID所需要的银行信息。 |
...
| `bankaccountblservice.BankAccountManagementBlService.delete(int id)` | 删除账户信息。 |
...
| EmployeeAddUiController         | 负责管理增加职员信息界面的展示和功能。   |
| EmployeeModifyUiController      | 负责管理修改职员信息界面的展示和功能。   |
| EmployeeSelectionUiController   | 负责财务人员选择职员信息界面的展示和功能。 |
| FaceIdRegistraitionUiController | 负责管理faceId注册的界面展示和功能。 |
...
| **EmployeeModificationUiController** 
提供的接口
...
**EmployeeSelectionUiController**

提供的接口

| 接口名称                      | 语法                                       | 前置条件        | 后置条件       |
| ------------------------- | ---------------------------------------- | ----------- | ---------- |
| adminui.EmployeeSelection | `public void showEmployeeSelectDialog (Consumer<List<EmployeeVo>> callback);` | 查询并选择有效的职员。 | 返回职员的详细信息。 |
| adminui.EmployeeSelection | `public EmployeeVo queryId(String id);`  | 根据id查找职员。   | 返回职员。      |
...
**FaceIdRegisterationUiController**
...
| 接口名称                                     | 服务名       |
| ---------------------------------------- | --------- |
| `adminblservice.FaceIdRegistrationBlService.register(String employeeId, byte[] image)` | 注册faceId。 |
| `adminui.EmployeeSelection.showEmployeeSelectDialog()` | 选择职员。     |
...
| PromotionManagementUiController | 负责管理填促销策略查看和删除界面的展示和功能。    |
| PromotionSelectionUiController  | 负责管理销售人员选择可用促销策略界面的展示和功能。  |
...
| 接口名称                             | 语法                                       | 前置条件             | 后置条件          |
| -------------------------------- | ---------------------------------------- | ---------------- | ------------- |
| ContinueWritable.continueWriting | `public ExternalLoadedUiPackage continueWriting(ComSalePromotionVo promotion);` | 输入有效的用于继续填写促销策略。 | 初始化传入促销策略的内容。 |
...
| 接口名称                             | 语法                                       | 前置条件             | 后置条件          |
| -------------------------------- | ---------------------------------------- | ---------------- | ------------- |
| ContinueWritable.continueWriting | `public ExternalLoadedUiPackage continueWriting(TotalPricePromotionVo promotion);` | 输入有效的用于继续填写促销策略。 | 初始化传入促销策略的内容。 |
...
| 接口名称                             | 语法                                       | 前置条件             | 后置条件          |
| -------------------------------- | ---------------------------------------- | ---------------- | ------------- |
| ContinueWritable.continueWriting | `public ExternalLoadedUiPackage continueWriting(ClientPromotionVo promotion);` | 输入有效的用于继续填写促销策略。 | 初始化传入促销策略的内容。 |
...
**PromotionManagementUiController**
...
**PromotionSelectionUiController**
提供的接口
| 接口名称                           | 语法                                       | 前置条件          | 后置条件         |
| ------------------------------ | ---------------------------------------- | ------------- | ------------ |
| promotionui.PromotionSelection | `public void showEmployeeSelectDialog (Consumer<PromotionVoBase> callback,SaleBillVo bill);` | 查询并选择可用的促销策略。 | 返回促销策略的详细信息。 |

##### 2.1.12.3.4 展示层的动态模型

下图为ComSalePromotion对象的状态图。其他两个促销策略对象同理。

![promotionui](./img/状态图/促销策略状态图.png)
...
| 接口名称                                     | 服务名     |
| ---------------------------------------- | ------- |
| `approvalblservice.AuditBlService.query(AuditIdQueryVO query)` | 查询单据信息。 |
| `approvalblservice.AuditBlService.reject(AuditIdVo billInfo)` | 拒绝通过审批。 |
| `approvalblservice.AuditBlService.pass(AuditIdVo billInfo)` | 通过审批。   |
...
| loginblservice.FaceIdAuthenticationBlService.authenticate | `public EmployeeVo authenticate (byte[] bytes);` | 参数为有效图片文件信息。 | 系统已经登录或者登录失败。 |
...
| `logindataservice.FaceIdAuthenticationDataService.authenticate(byte[] faceImage)` | 以图片登录。       |
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。        |
...
| InventoryGiftBlController      | 负责商品赠送功能以及为他人提供查询赠送单的实现。 |
| InventoryWarningBlController   | 负责库存监控功能以及为他人提供查询监控单的实现。 |
| PurchaseBillBlController       | 负责实现进货单的填写、修改、提交功能       |
| PurchaseBillRefundBlController | 负责实现进货退货单的填写、修改、提交功能     |
| PurchaseBillBlInfoController   | 负责实现为他人提供进/退货单的查询功能      |
...
| 接口名称                                     | 语法                                       | 前置条件        | 后置条件                                     |
| ---------------------------------------- | ---------------------------------------- | ----------- | ---------------------------------------- |
| InventoryCheckBlService.getInventoryView | `public InventoryViewVo getInventoryView(Date beginTime, Date endTime);` | 日期输入合法。     | 完成在此时间段内的出／入库数量／金额，销售／进货数量／金额，以及库存合计的查看。 |
| InventoryCheckBlService.getInventoryPicture | `public InventoryPictureVo getInventoryPicture();` | 已经得到当日库存快照。 | 生成截至盘点时间的库存盘点。                           |
| InventoryCheckBlService.export           | `public ResultMessage export(String path,String path);` | 已经完成库存盘点。   | 导出Excel到指定位置。                            |
...
| 接口名称                                     | 服务名        |
| ---------------------------------------- | ---------- |
| `saleblservice.SaleBillBlInfo.querySaleBill(SaleBillQueryVo saleBillQueryvo);` | 查询销售单。     |
| `logbl.LogBl.log(LogSeverity severity, String content)` | 记录日志。      |
| `saleblservice.SaleBillBlInfo.querySaleRefundBill(SaleBillRefundQueryVo saleBillRefundQueryvo);` | 查询销售退货单。   |
| `inventoryblservice.PurchaseBillBlInfo.queryPurchaseBillVo(PurchaseBillQueryVo purchaseBillQueryVo);` | 查询进货单。     |
| `inventoryblservice.PurchaseBillBlInfo.queryPurchaseRefundBillVo(PurchaseBillRefundQueryVo purchaseRefundBillQueryVo);` | 查询进货退货单。   |
| `util.PoVoConverter.fromVotoPo(CommodityVo vo);` | 将商品vo转为po。 |
...
| 接口名称                                     | 语法                                       | 前置条件          | 后置条件                             |
| ---------------------------------------- | ---------------------------------------- | ------------- | -------------------------------- |
| InventoryGiftBlService.getPromotionCommodity | `public CommodityItem[] getPromotionCommodity(String id);` | 输入合法。         | 提供符合条件的赠品。                       |
| InventoryGiftBlService.Sumbit            | `public ResultMessage sumbit(InventoryGiftVo inventoryGiftvo);` | 输入合法。         | 提交赠送单。                           |
| InventoryGiftBlService.saveAsDraft       | `public ResultMessage saveAsDraft(InventoryBillVo bill);` | 单据信息非空。       | 保存草稿，持久化信息已经保存。                  |
| InventoryGiftBlService.getId             | `public String getId();`                 | 当前存在赠送单。      | 提供当前赠送单ID。                       |
| InventoryGiftBlService.query             | `public InventoryGiftVo[] query(InventoryGiftQueryVo inventoryBillQueryVo);` | 输入合法。         | 查询符合条件的赠送单。                      |
| InventoryGiftInfo.queryInventoryGiftBill | `public InventoryGiftVo[] queryInventoryGiftBill(InventoryGiftQueryVo inventoryBillQueryVo);` | 输入合法。         | 查询符合条件的赠送单。                      |
...
| util.BillPoVoConverter.fromPoToVo        | `InventoryGiftVo fromPoToVo(InventoryGiftPo po);` | 无。            | po转vo。                           |
| util.BillPoVoConverter.fromVoToPo        | `InventoryGiftPo fromVoToPo(InventoryGiftVo vo);` | 无。            | vo转po。                           |
...
| `inventorydataservice.InventoryGiftDataService.submit(InventoryGiftBillPO giftBill);` | 提交表单。                          |
| `saleblservice.SaleBillBlInfo.querySaleBill(SaleBillQueryVo vo);` | 查询销售单。                         |
| `inventorydataservice.InventoryGiftDataService.getId();` | 获得新单据的ID。                      |
| `inventorydataservice.InventoryGiftDataService.query(InventoryGiftQueryPo query);` | 查询已有赠送单。                       |
| `inventorydataservice.InventoryGiftDataService.activate(BillPo bill) ` ClientModificationService | 使单据生效。                         |
| `inventorydataservice.InventoryGiftDataService.abandon(BillPo bill)` | ClientModificationService废弃单据。 |
| `inventorydataservice.InventoryGiftDataService.approvalComplete(String billId, BillState state)` | 审批完成，修改单据状态。                   |
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。                          |
| `draftbl.DraftService.saveAsDraft(BillPo bill)` | 保存草稿。                          |
| `clientbl.ClientModificationService.modifyClient(String clientId, ClientModificationFlag flag, double delta)` | 修改用户信息。                        |
| `approvalbl.ApprovalRequest.requestApproval(BillVo vo)` | 提交等待审核。                        |
...
| 接口名称                                   | 语法                                       | 前置条件              | 后置条件                             |
| -------------------------------------- | ---------------------------------------- | ----------------- | -------------------------------- |
| InventoryWarningBlService.submit       | `public ResultMessage submit(InventoryDetailBillVo bill);` | 单据所有属性有效。         | 单据已经保存到数据库，持久化信息已经保存。            |
| InventoryWarningBlService.saveAsDraft  | `public ResultMessage saveAsDraft(InventoryBillVo bill);` | 单据信息非空。           | 保存草稿，持久化信息已经保存。                  |
| InventoryWarningBlService.modify       | `public ResultMessage modify(String id，double modifyWarning);` | 该商品存在，且修改警戒值输入合法。 | 修改警戒值，持久化更新涉及的对象的数据。             |
...
| DraftDeleteService.deleteDraft         | `public ResultMessage deleteDraft(String id);` | 无。                | 系统删除草稿，持久化信息已经保存。                |
| approvalbl.BillApprovalCompleteService | `ResultMessage approvalComplete(String billId, BillState state);` | 无。                | 系统修改单据状态，持久化信息已经保存。              |
| util.BillPoVoConverter.fromPoToVo      | `InventoryDetailBillVo fromPoToVo(InventoryDetailBillPo po);` | 无。                | po转vo。                           |
| util.BillPoVoConverter.fromVoToPo      | `InventoryDetailBillPo fromVoToPo(InventoryDetailBillVo vo);` | 无。                | vo转po。                           |
...
| `commodityblservice.CommodityInfo.update(String id，double ModifyWarning)` | 修改警戒值。       |
| `inventorydataservice.InventoryWarningDataService.query(InventoryBillQueryVo inventoryBillQueryVo)` | 查询符合条件单据。    |
| `inventoryblservice.InventoryModificationService.modifyInventory(String commodityId, InventoryModificationFlag flag, double delta);` | 无。           |
| `inventorydataservice.InventoryWarningDataService.getId();` | 获得新单据的ID。    |
| `inventorydataservice.InventoryWarningDataService.activate(BillPo bill) ` | 使单据生效。       |
| `inventorydataservice.InventoryWarningDataService.abandon(BillPo bill)` | 废弃单据。        |
| `inventorydataservice.InventoryWarningDataService.approvalComplete(String billId, BillState state)` | 审批完成，修改单据状态。 |
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。        |
| `draftbl.DraftService.saveAsDraft(BillPo bill)` | 保存草稿。        |
| `notificationbl.NotificationBlService.acknowledge(OtherNotificationVo otherNotificationVo)` | 发送通知。        |
| `adminbl.EmployeeInfo.queryById(String id);` | 查询身份id。      |
| `clientbl.ClientModificationService.modifyClient(String clientId, ClientModificationFlag flag, double delta)` | 修改用户信息。      |
| `approvalbl.ApprovalRequest.requestApproval(BillVo vo)` | 提交等待审核。      |
...
| `approvalbl.ApprovalRequest.requestApproval(BillVo vo)` | 提交等待审核。      |
| `client.ClientModificationService.modifyClient(String clientId, ClientModificationVo vo)` | 提交等待审核。      |
...
| SaleBillInfoController     | 负责实现为他人提供查询销售、退货单功能  |
...
| SaleBillBlService.queryPromotion       | `public PromotionVoBase[] queryPromotion(SaleBillVo saleBillVo); ` | 无。            | 获得当前可用的销售策略。                     |
| NotificationActivateService.activate   | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。 | 系统修改对应客户信息，修改单据状态为已入账，持久化信息已经保存。 |
| NotificationAbandonService.abandon     | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。 | 系统修改单据状态为已经废弃，持久化信息已经保存。         |
| DraftDeleteService.deleteDraft         | `public ResultMessage deleteDraft(String id);` | 无。            | 系统删除草稿，持久化信息已经保存。                |
| approvalbl.BillApprovalCompleteService | `ResultMessage approvalComplete(String billId, BillState state);` | 无。            | 系统修改单据状态，持久化信息已经保存。              |
...
| `promotionbl.PromotionInfoBl.getPromotion(SaleBillVo);` | 根据销售单获得可用的促销策略。 |
...
| CommodityInfo                | 负责实现为其他包提供查询获取商品信息的服务   |
| CommoditySortInfo            | 负责实现为其他包提供查询获取商品分类信息的服务 |
| InventoryModificationService | 负责实现为他人提供修改库存的服务。       |
...
| CommodityBlService.queryNormally   | `public CommodityVo[] queryNormally(String query);` | 查询输入格式合法。   | 返回模糊化查询的数据。           |
| CommoidtyBlService.getId           | `public String getId(String sortId);`    | 无。          | 返回新商品id。              |
| CommodityBlService.queryCommodity  | `public CommodityVo[] queryCommodity(CommodityQueryVo commodityQueryVo)` | 查询输入个是合法。   | 返回符合条件的商品。            |
| CommoidtyBlService.update          | `public ResultMessage update(String id, double value);` | id合法。       | 修改警戒值。                |
| CommoidtyBlService.getAllCommodity | `public CommodityVo[] getAllCommodity();` | 无。          | 获取所有商品。               |
| CommoidtyBlService.modifyInventory | `public ResultMessage modifyInventory(String commodityId, InventoryModificationFlag flag, double delta) ;` | id合法        | 修改库存。                 |
...
| `commoditydataservice.CommodityDataService.queryNormally(String query)` | 删除单一化持久对象。          |
| `logbl.LogBl.log(LogSeverity severity, String content)` | 记录日志。               |
| `commoditydataservice.CommodityDataService.getId()` | 得到新商品id。            |
...
| CommoditySortBlService.getId             | `public String getId();`                 | w无。                  | 得到新分类id。              |
| CommoditySortBlService.queryCommoditySort | `public CommoditySortVo[] queryCommoditySort(CommoditySortQuerVo commoditySortQueryVo);` | 商品分类查询信息输入合法。        | 返回符合查询条件的数据。          |
| CommoditySortBlService.getAllCommoditySort | `public CommoditySortVo[] getAllCommoditySort();` | 无。                   | 返回所有商品分类。             |
| `logbl.LogBl.log(LogSeverity severity, String content)` | 记录日志。                                    |                      |                       |
...
| `commoditydataservice.CommoditySortDataService.getId()` | 得到新id。          |
| `logbl.LogBl.log(LogSeverity severity, String content)` | 记录日志。           |
...
| CommodityInfo.update         | ` ResultMessage update (String id,double value);` | id合法。      | 修改警戒值。        |
| CommodityInfo.modify         | ` ResultMessage modify(CommodityVo commodityVo);` | 输入数据合法。    | 修改商品。         |

需要的接口

| 接口名称                                     | 服务名                 |
| ---------------------------------------- | ------------------- |
| `commodityBlService.CommodityBlService.query(CommodityQueryVo commodityQueryVo)` | 根据ID和/或名称模糊化查询商品分类。 |
| `commodityBlSerevice.CommodityBlService.modify(CommodityVo commodityVo)` | 修改商品。               |
...
| 接口名称                                 | 语法                                       | 前置条件         | 后置条件            |
| ------------------------------------ | ---------------------------------------- | ------------ | --------------- |
| CommoditySortInfo.queryCommoditySort | `public CommodityVo[] queryCommoditySort(CommoditySortQueryVo commoditySortQueryVo);` | 其他类需要商品分类信息。 | 返回所需商品分类的持久化对象。 |
...
**InventoryModificationService**

提供的接口

| 接口名称                                     | 语法                                       | 前置条件  | 后置条件    |
| ---------------------------------------- | ---------------------------------------- | ----- | ------- |
| InventoryModificationService.modifyInventory | `ResultMessage modifyInventory(String commodityId, InventoryModificationFlag flag, double delta);` | id合法。 | 修改库存数量。 |
...
| 接口名称                                     | 服务名   |
| ---------------------------------------- | ----- |
| `commodityBlService.CommodityBlService.modify(CommodityVo commodityVo)` | 修改商品。 |
...
| ClientBlService.queryById              | `public ClientVo queryById(String id);`  | 查询id合法。   | 返回符合id的客户信息。          |
...
| `clientdataservice.ClientDataService.queryById(String id)` | 根据id查询客户信息。   |
...
| NotificationAbandonService.abandon     | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。 | 系统修改单据状态为已经废弃，持久化信息已经保存。              |
| DraftDeleteService.deleteDraft         | `public ResultMessage deleteDraft(String id);` | 单据有效且状态为草稿。   | 系统删除草稿单据。                             |
...
| BillIdQueryService.queryBill     | `public BillVo queryBill(String id);`    | 无。                           | 返回ID为参数的单据Vo。                |
...
| `inventorybl.PurchaseBillInfo.queryPurchaseRefundBill(PurchaseRefundBillQueryVo query)` | 查询进货退货单。     |
| `inventorybl.InventoryBillInfo.queryInventoryBill(InventoryBillQueryVo query)` | 查询库存类单据。     |
| `promotionbl.couponbl.CouponInfo.queryUnusedCouponValue(Date strat, Date end)` | 查询未使用的代金券总额。 |
...
| BankAccountManagementBlService.delete    | `public ResultMessage delete(int id);`   | 选择的账户合法，用户具有最高权限。     | 选定账户信息已经删除，持久化信息已经保存。  |
| BankAccountInfo.queryBankAccount         | `public BankAccountVo[] queryBankAccount(BankAccoutQueryVo query);` | 无。                    | 返回符合查询条件的数据。           |
| BankAccountModificationService.modifyBankAccount | `public ResultMessage modityBankAccount(int accountId, double delta);` | 新银行账户Vo的ID存在于原数据库。    | 修改银行账户信息。              |
...
| `bankaccountdataservice.BankAccountManagementDataService.delete(int id);` | 删除账户信息。        |
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。          |
...
| logblservice.LogBackupBlService.backupLog | `public ResultMessage backupLog()`       | 用户已经登录，具有最高权限。 | 备份操作日志。         |
| logblservice.LogBackupBlService.fetchCloudLog | `public String fetchCloudLog()`          | 用户已经登录，具有最高权限。 | 返回云上的操作日志。      |
...
| `loginbl.CurrentUserService.getCurrentUser()` | 获得当前登录的用户。    |
| `logdataservice.LogBackupDataService.backupLog()` | 清空本地日志，存储在云上。 |
| `logdataservice.LogBackupDataService.fetchCloudLog()` | 返回云上日志的URL地址。 |
...
| FaceIdRegistrationBlController | 负责FaceId的注册。 |
...
| FaceIdRegistrationBlService.register   | `public ResultMessage register(String employeeId, byte[] image)` | 用户ID有效，图片信息有效。        | 注册或者更新图片信息，持久化信息已经保存。 |
| FaceIdRegistrationBlService.unregister | `public ResultMessage unregister(String employeeId)` | 用户ID有效。               | 删除用户的人脸信息，持久化信息已经保存。  |
...
**FaceIdRegistrationBlController**

提供的接口

| 接口名称                                   | 语法                                       | 前置条件                  | 后置条件                 |
| -------------------------------------- | ---------------------------------------- | --------------------- | -------------------- |
| FaceIdRegistrationBlService.register   | `public ResultMessage register(String employeeId, byte[] image);` | 新账户信息合法且无冲突，用户具有最高权限。 | 新账户信息已经添加，持久化信息已经保存。 |
| FaceIdRegistrationBlService.unregister | `public ResultMessage unregister(String employeeId);` | 新账户信息合法且无冲突，用户具有最高权限。 | 新账户信息已经添加，持久化信息已经保存。 |
...
| 接口名称                                     | 服务名              |
| ---------------------------------------- | ---------------- |
| `admindataservice.FaceIdRegistrationDataService.register(String employeeId, byte[] image)` | 根据ID和图像注册faceId。 |
| `admindataservice.FaceIdRegistrationDataService.unregister(String employeeId)` | 删除faceId。        |
...
| 类名                          | 职责          |
| --------------------------- | ----------- |
| AuditBlController           | 负责实现审批。     |
| ApprovalRequestBlController | 负责实现单据请求审批。 |
...
| 接口名称                           | 语法                                       | 前置条件          | 后置条件                  |
| ------------------------------ | ---------------------------------------- | ------------- | --------------------- |
| AuditBlService.query           | `public ResultMessage query (AuditVoQueryVO query);` | 有处于提交状态的单据单据。 | 返回符合条件的单据。            |
| AuditBlService.reject          | `public ResultMessage reject(AuditIdVO bill);` | 用户已经查看了单据内容。  | 拒绝通过审批，持久化更新涉及的对象的数据。 |
| AuditBlService.pass            | `public ResultMessage pass(AuditIdVO bill);` | 用户已经查看了单据内容。  | 通过审批，持久化更新涉及的对象的数据。   |
| ApprovalRequest.requestMessage | `public ResultMessage requestMessage(AuditIdVo bill);` | 用户提交了单据。      | 保存单据并返回保存结果。          |

需要的接口

| 接口名称                                     | 服务名          |
| ---------------------------------------- | ------------ |
| `approvaldataservice.AuditDataService.requestApproval(AuditIdQueryVo query)` | 得到待审批的持久化对象。 |
| `approvaldataservice.AuditDataService.reject(AuditIdVo billInfo)` | 拒绝通过审批。      |
| `approvaldataservice.AuditDataService.pass(AuditIdVo billInfo)` | 通过审批。        |
...
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。        |
| `auditbl.BillApprovalCompleteService.approvalComplete(String billId, BillState state)` | 修改单据状态。      |

**ApprovalRequestBlController**
提供的接口

| 接口名称                       | 语法                                       | 前置条件    | 后置条件    |
| -------------------------- | ---------------------------------------- | ------- | ------- |
| approvalbl.ApprovalRequest | `public ResultMessage query (BillVO bill);` | 有单据被提交。 | 返回提交结果。 |

需要的接口

| 接口名称                                     | 服务名          |
| ---------------------------------------- | ------------ |
| `approvaldataservice.AuditDataService.query(AuditIdQueryVo query)` | 得到待审批的持久化对象。 |
| `logbl.LogService.log(LogSeverity severity, String content)` | 记录日志。        |
...
| FaceIdAuthenticationDataController | 提供人脸识别的接口。   |
...
| 接口名称                                     | 说明   |
| ---------------------------------------- | ---- |
| `admindata.LoginService.login(String username, String password)` | 登录。  |

**FaceIdAuthenticationDataController**

提供的接口

| 接口名称                                     | 语法                                       | 前置条件    | 后置条件                    |
| ---------------------------------------- | ---------------------------------------- | ------- | ----------------------- |
| logindataservice.FaceIdAuthenticationDataService.authenticate | `public String authenticate(byte[] image)` | 图片信息有效。 | 系统已经登录或者登录失败，返回用户登录的ID。 |

需要的接口

| 接口名称                                     | 说明        |
| ---------------------------------------- | --------- |
| `faceid.FaceIdAuthenticationService.authenticate(byte[] image)` | 使用人脸信息登录。 |
...
| 接口名称                              | 语法                                       | 前置条件          | 后置条件                                  |
| --------------------------------- | ---------------------------------------- | ------------- | ------------------------------------- |
| InventoryGiftDataService.activate | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。 | 系统修改对应库存信息和客户信息，修改单据状态为已入账，持久化信息已经保存。 |
| InventoryGiftDataService.abandon  | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。 | 系统修改单据状态为已经废弃，持久化信息已经保存。              |
| InventoryGiftDataService.query    | `public PurchaseBillPo[] query(InventoryGiftQueryVo query);` | 输入查询条件有效。     | 返回符合筛选条件的进货单。                         |
| InventoryGiftDataService.submit   | `public ResultMessage submit(InventoryGiftPo inventoryGiftPo);` | 表单数据输入格式正确。   | 持久化层新增表单信息。                           |
| InventoryGiftDataService.getId    | `public String getId();`                 | 无。            | 持久化层新单据的ID。                           |
...
| 接口名称                                 | 语法                                       | 前置条件          | 后置条件                                  |
| ------------------------------------ | ---------------------------------------- | ------------- | ------------------------------------- |
| InventoryWarningDataService.activate | `public ResultMessage activate(String id);` | 单据有效且状态为审批通过。 | 系统修改对应库存信息和客户信息，修改单据状态为已入账，持久化信息已经保存。 |
| InventoryWarningDataService.abandon  | `public ResultMessage abandon(String id);` | 单据有效且状态为审批完成。 | 系统修改单据状态为已经废弃，持久化信息已经保存。              |
| InventoryWarningDataService.query    | `public PurchaseBillPo[] query(InventoryGiftQueryVo query);` | 输入查询条件有效。     | 返回符合筛选条件的进货单。                         |
| InventoryWarningDataService.submit   | `public ResultMessage submit(InventoryGiftPo inventoryGiftPo);` | 表单数据输入格式正确。   | 持久化层新增表单信息。                           |
| InventoryWarningDataService.getId    | `public String getId();`                 | 无。            | 持久化层新单据的ID。                           |
...
| CommodityDataService.queryNormally   | `public CommodityPo[] queryNormally(String query);` | 查询输入格式合法。     | 返回符合查询条件的数据。 |
| CommodityDataService.getId           | `public String getId();`                 | 无。            | 返回新id。       |
| CommodityDataService.getAllCommodity | `public CommodityPo[] getAllCommodity();` | 无。            | 返回所有商品。      |
...
| CommoditySortDataService.getId   | `public String getId();`                 | 无。            | 返回新id。       |
...
| ClientDataService.queryById   | `public ClientPO query(String id);`      | 查询id合法。     | 返回符合id的用户信息。 |
...
| BankAccountManagementDataService.delete | `public ResultMessage delete(int id);`   | 新账户信息合法且无冲突。 | 选定账户信息已经删除，持久化信息已经保存。            |
...
| logdataservice.LogBackupDataService.backupLog | `public ResultMessage backupLog();`      | 无。         | 清空本地日志，存储在云上。   |
| logdataservice.LogBackupDataService.fetchCloudLog | `public ResultMessage fetchCloudLog();`  | 无。         | 返回云上日志的URL地址。   |
...
| FaceIdRegistrationDataController | 负责更新、注册和删除人脸信息的数据。 |
...
**FaceIdRegistrationDataController**

提供的接口

| 接口名称                                     | 语法                                       | 前置条件         | 后置条件                     |
| ---------------------------------------- | ---------------------------------------- | ------------ | ------------------------ |
| FaceIdRegistrationDataService.register   | `public ResultMessage register(String employeeId, byte[] image);` | ID有效，图片信息有效。 | 注册或者更新用户的人脸信息，持久化信息已经保存。 |
| FaceIdRegistrationDataService.unregister | `public ResultMessage unregister(String employeeId);` | ID有效。        | 删除用户的人脸信息，持久化信息已经保存。     |

需要的接口

| 接口名称                                     | 说明           |
| ---------------------------------------- | ------------ |
| `faceid.FaceIdRegistrationService.register(String employeeId, byte[] image)` | 使用人脸信息注册。    |
| `faceid.FaceIdRegistrationService.unregister(String employeeId)` | 取消对应用户的人脸信息。 |
...
| 接口名称                                | 语法                                       | 前置条件              | 后置条件            |
| ----------------------------------- | ---------------------------------------- | ----------------- | --------------- |
| ApprovalDataService.query           | `public AuditVO[] query(AuditQueryVo query)` | po中存在处于提交状态的单据数据。 | 返回待审批单据。        |
| ApprovalDataService.reject          | `public ResultMessage reject(AuditIdVo billInfo);` | 无。                | 修改并保存单据持久化对象数据。 |
| ApprovalDataService.pass            | `public ResultMessage pass(AuditIdVo billInfo);` | 无。                | 修改并保存单据持久化对象数据。 |
| ApprovalDataService.requestApproval | `public ResultMessage requestApproval(AuditIdVo billInfo);` | 有单据被提交。           | 返回保存结果。         |
...
faceid包提供了所有人脸验证登录、注册、更新和删除所需要的服务。具体功能需求和非功能需求可参见需求规格说明文档和体系结构设计文档。

#### 2.3.15.2 整体架构

此包为数据层的一部分，负责所有人脸验证登录、注册、更新和删除所需要的服务。它实现了FaceIdAuthenticationService和FaceIdRegistrationService接口。

#### 2.3.15.3 设计

##### 2.3.15.3.1 设计图

![faceid](./img/设计图/faceid.png)

##### 2.3.15.3.2 各个类的职责

| 类名                             | 职责                 |
| ------------------------------ | ------------------ |
| FaceIdAuthenticationController | 负责人脸识别验证。          |
| FaceIdRegistrationController   | 负责人脸识别信息的注册、更新和删除。 |

##### 2.3.15.3.3 内部类的接口规范

**FaceIdAuthenticationController**

提供的接口

| 接口名称                                     | 语法                                       | 前置条件    | 后置条件                      |
| ---------------------------------------- | ---------------------------------------- | ------- | ------------------------- |
| FaceIdAuthenticationService.authenticate | `public String authenticate(byte[] img)` | 图片信息有效。 | 成功登录或者登录失败，返回图片信息对应的职员id。 |

**FaceIdRegistrationController**

提供的接口

| 接口名称                                 | 语法                                       | 前置条件         | 后置条件                    |
| ------------------------------------ | ---------------------------------------- | ------------ | ----------------------- |
| FaceIdRegistrationService.register   | `public ResultMessage register(String employeeId, byte[] img)` | ID有效，图片信息有效。 | 更新或者注册职员人脸信息，持久化信息已经保存。 |
| FaceIdRegistrationService.unregister | `public ResultMessage unregister(String employeeId)` | ID有效。        | 删除职员的人脸信息。              |
