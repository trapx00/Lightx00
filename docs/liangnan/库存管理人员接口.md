...
| Inventorydataservice.InventoryCheck.picture； | `public InventoryPicturePO submit(Date time);` | 当天有库存快照。      | 返回对应的InventoryPicturePO。 |
| Inventorydataservice.InventoryCheck.initiation； | `public void init()；`                    | 无。            | 初始化持久数据。                 |
